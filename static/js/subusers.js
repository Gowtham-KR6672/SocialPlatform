    ...
    return `<div class="user-card">
        <div class="uc-top"><span class="uav ph">${esc((su.display_name||su.username).slice(0,2).toUpperCase())}</span>
          <div class="uc-id"><div class="uc-name">${esc(su.display_name||su.username)} <span class="pdot ${su.online?'on':'off'}"></span></div>
            <div class="uc-role"><span class="urole-badge role-subuser">Sub-User</span> · @${esc(su.username)}</div></div></div>
        <div class="uc-svcs" style="flex-direction:column;align-items:flex-start;gap:4px">
          <span class="usvc">Sections: ${esc(tabs)}</span>
          <span class="usvc">Editing: ${su.can_edit?'Allowed':'View only'}</span>
...
function openSubuserModal(su){
  ...
  const m = el(`<div class="modal">
    ...
    <div class="modal-body">
      ${editing?'':`<label class="f">Username</label><input class="f" id="su-user" placeholder="e.g. editor1" autocomplete="off">
      <label class="f">Password</label><input class="f" id="su-pass" type="password" placeholder="Set a password">`}
  ...
  $('#su-save',m).onclick = async ()=>{
    ...
    const can_edit = $('#su-edit',m).checked;
    ...
    try{
      if(editing){
        await api('/api/subusers/'+su.id,{method:'PATCH', body:{
          access_tabs, can_edit, allowed_videos,
        ...
        const body = {username:$('#su-user',m).value.trim(), password:$('#su-pass',m).value,
          display_name:$('#su-name',m).value.trim(), access_tabs, can_edit, allowed_videos};
        await api('/api/subusers',{method:'POST', body});
        toast('Sub-User created','good');
      }
